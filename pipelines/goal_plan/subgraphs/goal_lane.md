# goal_lane.dot — real-work lane brick

Real-work sibling of `goal_lane_smoke.dot`. Same bounded child-convergence
shape, but the attempt does **real work** against a referenced goal instead
of writing a fixed marker string.

## What changed vs `goal_lane_smoke.dot`

| Concern | `goal_lane_smoke.dot` (marker) | `goal_lane.dot` (real work) |
|---|---|---|
...
- Writes `$lane_result_path` as `{"result": "candidate", "candidate_sha": "<sha>"}`
  ...
  `goal_lane_smoke.dot`'s `Candidate` node.
- Keeps `goal_lane_smoke.dot`'s budget-ledger params (`$runtime_py_dir`,
...
## Contract (identical to `goal_lane_smoke.dot`)
