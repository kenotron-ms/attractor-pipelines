...
| `deliver_pr.dot`'s `OpenPR` node (shared by `hello_world`, `idea_to_shipped`, `ship_ready`, and inlined in `idea_to_pr`) | `OpenPR` was prompted to "open a pull request" and write the resulting URL to a file; `ExtractPRUrl` then read that file and immediately set `delivery.result="opened"` -- trusting the URL was real with zero independent check. An LLM node can print a plausible-looking PR URL, or `gh`/`curl` can fail after emitting output the agent misreads, with no PR actually existing on GitHub | Added `CheckPROpened`, a deterministic `parallelogram` gate after `ExtractPRUrl` that independently confirms the PR exists via `gh pr view` (falling back to a direct GitHub API `curl` call) before a separate `MarkOpened` node is allowed to set `delivery.result="opened"`. **Portability note:** deliberately built from `gh`/`curl` only, never a platform-specific tool (e.g. a Resolve worker-container helper) -- these pipelines are fetched and run via `git+https://` URLs by any dot-graph-compatible engine, not exclusively one platform's runtime, so a platform-only verification primitive would silently break elsewhere |
